...
public interface DaoCallery {
    ...
    @Insert("insert into gallery(categoryId,name,creater,price,createTime,updateTime,image,descroption,details) " +
            "values(#{categoryId},#{name},#{creater},#{price},#{createTime},#{updateTime},#{image},#{descroption},#{details})")
...
//    public String cate(int id);
     ...
     * @return
     */
    @Select("select g.*,c.name cName from gallery g left join category c on g.categoryId = c.id")
    @Results(id = "galleryRelt", value = {
            @Result(id = true, column = "id", property = "id"),
            @Result(column = "categoryId", property = "categoryId"),
            @Result(column = "name", property = "name"),
