        ...
        if (ServletFileUpload.isMultipartContent(request)) {
            ...
            while (it.hasNext()) {
                ...
                if (fileItem.isFormField()) {
                    ...
                    }else if (fileName.equals("sourceType")) {
...
//                        String rootPath = request.getServletContext().getRealPath("/");
                    ...
                    String path = realImagePathRelease+"/image/gallery/"+date+imageType;
                    fileItem.write(new File(path));

                    calloryItem.setImgPath("/image/gallery/"+date+imageType);
...
//    }
    ...
    public void galleryDelete(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        CalloryItem calloryItem = daoCallery.selectCalloryById(ID);

        deleteImg(calloryItem);

        daoCallery.deleteGallery(ID);

        galleryList(request, response);

    }

    public void deleteImg(CalloryItem calloryItem){
        File file= new File(realImagePathRelease+calloryItem.getImgPath());
            ...
            while (it.hasNext()) {
                ...
                if (fileItem.isFormField()) {
                    ...
                    if (fileItem.get().length == 0) {
                        ...
                        deleteImg(daoCallery.selectCalloryById(Integer.parseInt(id)));
//                        calloryItem.setImage(fileItem.get());
...
//                        String rootPath = request.getServletContext().getRealPath("/");
            ...
            daoCallery.updataGallery(calloryItem);
